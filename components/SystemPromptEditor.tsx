...
export default function SystemPromptEditor() {
  ...
  const loadPrompt = async () => {
    try {
      const res = await fetch('/WA/api/system-prompt');
  ...
  const savePrompt = async () => {
    ...
    try {
      await fetch('/WA/api/system-prompt', {
        method: 'POST',
            ...
            <button
              ...
              className={`px-3 py-1.5 text-sm text-white rounded-md flex items-center space-x-1 ${
                saved 
                  ? 'bg-green-600' 
                  : hasChanges 
                    ? 'bg-pixan-primary hover:bg-pixan-dark' 
...
Eres un asistente útil y amigable de pixan. Respondes de forma concisa y profesional. Siempre saludas con entusiasmo y ayudas a los usuarios con sus preguntas."
          className="w-full h-full p-4 border border-gray-200 rounded-lg resize-none focus:outline-none focus:ring-2 focus:ring-pixan-primary focus:border-transparent font-mono text-sm"
        />
      </div>
